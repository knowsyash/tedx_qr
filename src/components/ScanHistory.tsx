        ...
        <>
          <div className="max-h-60 overflow-y-auto">
            {history.map((scan, index) => (
              <div 
                key={index}
                className={`flex items-center p-3 border-t border-gray-100 ${
                  index % 2 === 0 ? 'bg-gray-50' : 'bg-white'
                }`}
              >
                <div className={`flex-shrink-0 mr-3 rounded-full p-1.5 ${
                  scan.isValid ? 'bg-green-100 text-green-600' : 'bg-red-100 text-[#e62b1e]'
                }`}>
